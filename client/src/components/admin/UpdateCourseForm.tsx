...
interface IUpdateCourseForm {
  open: boolean;
  onClose: () => void;
  initialValues: CreateCourseFormFields;
}
...
const UpdateCourseForm: React.FC<IUpdateCourseForm> = ({
  open,
  onClose,
  initialValues,
}) => {
  ...
  const { mutate, isLoading } = useMutation(UpdateCourseMutation, {
    onSuccess: () => {
      queryClient.invalidateQueries("get-all-courses").then(() => {
        showSuccess("Course was successfully updated");
        onClose();
      });
    },
    onError: (e) => {
  ...
  return (
    <>
      <FormWrapper<CreateCourseFormFields>
        ...
        dismissLabel={"CLOSE"}
        subHeader={
          "Update courses to schdule classes and allow students to buy the courses"
        }
